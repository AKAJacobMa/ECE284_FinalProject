...
def objective(trial):
    ...
    clf = make_pipeline(
        SVC(C=C, gamma=gamma, kernel=kernel, degree=degree)
    )
...
if __name__ == "__main__":
    ...
    if 'degree' not in best_params:
        best_params['degree'] = 3 # Default degree for non-poly kernels
    
    best_model = make_pipeline(
        SVC(C=best_params['C'], gamma=best_params['gamma'], kernel=best_params['kernel'], degree=best_params['degree'], random_state=42)
    )
